...
model.load_state_dict(torch.load("YOLIC_M2.pth.tar"))
    ...
    with torch.no_grad():
        for img_path in os.listdir(img_folder):
            if img_path.endswith(('.jpg', '.png', '.jpeg')):
                ...
                # resize_image = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
                input_tensor = preprocess(resize_image)
                input_batch = input_tensor.unsqueeze(0)
                output = model(input_batch)
                output = torch.sigmoid(output)
                target = torch.zeros((NumCell, NumClass + 1))
                frame = pred_plot(img, torch.Tensor.cpu(target), output[0])
